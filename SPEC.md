...
| **Aucun aperçu** | Température, tolérance, seuils d'arrêt, budget de temps | rien à montrer |
...
| Reconstruction d'un aperçu 17×17 | impossible | 5,3 ms |
...
- **JSON** pour la persistance : lisible, modifiable à la main, comparable dans git.
- **pytest** : tests.
...
| | Débit | Coût |
|---|---|---|
| Via `cdist` (actuel) | 473 000 coutures/s | aucun |
...
**Interface** : une barre de préréglages, puis les trois écrans :
...
2. Paramètres : les pages, la famille « Grille » (presets d'impression, aperçu fil
   de fer, cases vides cliquables) et la famille « Algorithme » (recherche et
   paramètres avancés, avec aperçus et projections chiffrées)
3. Vue d'exécution : image live, timeline navigable au clavier, zoom,
...
962 tests. Interface auditée en entier, en deux lots, `ui/` d'abord, puis la vue
...
| 2026-09-02 | **On ne quitte pas l'étape 1 sans cartes, ni pendant qu'elle en charge.** Les cartes arrivent par lots : dès le premier, le jeu n'était plus vide et « Suivant » s'allumait, alors que la moitié des extensions manquait encore. On passait alors à l'étape 2 pour y dimensionner une grille sur un nombre de cartes qui montait derrière, et la grille proposée au premier passage ne se propose **qu'une fois**. Un téléchargement en cours compte pareil : il se termine par un chargement. ⚠️ Le verrou tient à un **drapeau**, non à l'état du fil : on ne lâche pas la référence d'un `QThread` inachevé, et `isRunning()` peut être encore vrai à l'instant où le chargement s'annonce fini, le bouton serait resté éteint sans plus rien pour le rallumer |
| 2026-09-02 | **Les onglets de l'étape 2 portent la couleur de leur état**, libellé compris : la pastille seule tient sur vingt-deux pixels au bord de la colonne, et sur un écran large l'œil est à l'autre bout. Trois états et non plus deux : ⚠️ **la croix rouge est réservée à ce qui bloque**, des cartes qui ne tiennent pas dans les pages, des cases vides pas toutes posées. L'ambre dit « pas encore fait », ce qui n'est pas une faute : au premier passage tout est ambre, et un écran de croix rouges accueillerait l'utilisateur comme un formulaire raté. Repliée, la famille « Grille » prend le **pire** état de ses trois onglets : une croix cachée sous un groupe fermé serait exactement ce qu'on ne veut pas rater |
| 2026-09-05 | **Le dialogue d'export se dégorge : il ne garde que l'écriture.** Format, **dossier** et **nom de base**, les feuilles y ajoutant « page1 », « page2 » ; la qualité JPEG reste, elle appartient au format. ⚠️ **Le chemin se coupe en deux** : un champ unique mélangeait dossier et nom, et l'extension y menait sa vie, on pouvait laisser « .png » en ayant choisi JPEG. Les noms de fichiers passent de « poster_1of2 » à **« poster_page1sur2 »**, et gardent leur ligne et colonne sur plusieurs rangées. Le reste **remonte dans les onglets**, devant l'image : un onglet **« Coupe »** neuf, entre les couleurs et la résolution, porte le chevauchement et les repères, inertes quand il n'y a qu'une feuille ; **« Images d'origine »** rejoint la résolution. ⚠️ **La finesse s'arrête au maximum utile**, et une case « Aller plus loin » lève la borne : on montait à 1200 dpi sans rien gagner qu'un fichier quatre fois plus lourd. La source commande ce maximum, les vignettes valant le quart des images d'origine. ⚠️ Le dépassement se compare à la valeur **arrondie**, celle qui borne le champ : posé exactement sur le plafond, on lisait qu'on était déjà au-delà |
| 2026-09-05 | ⚠️ **« Ajuster » devient un mode, et rend la souris à la grille.** Le bouton garde son mot mais reste **enfoncé**, en bleu : tant qu'il l'est, le clic tiré déplace la mosaïque **dans sa feuille**, comme à l'onglet « Emplacement de la grille », curseur en main comprise ; éteint, il rend le glissement à la vue, qui se promène dans l'image. Les deux gestes sont le même, cliquer et tirer : il faut donc dire lequel on veut, et un bouton enfoncé le dit tout du long. L'ajustement du zoom, qu'il faisait avant, se tape dans son champ, à 100 %. ⚠️ Le cadre défilant **cesse d'intercepter** le clic dans ce mode, sans quoi l'aperçu ne le recevrait jamais |
| 2026-09-05 | **L'export dit ce qui déborde, et son bouton porte l'aura.** La grille est arrêtée, mais l'écart, la taille des cartes et le nombre de feuilles se règlent encore là : on pouvait, d'un clic, pousser des cartes hors des pages sans que rien ne le dise. Une phrase rouge le dit et **le bouton d'export se grise** : écrire un poster amputé n'a pas de sens. Le bouton porte l'**aura** qu'avait « Suivant », verte quand on peut écrire, rouge et plus discrète quand il manque quelque chose. **« Suivant » disparaît à la dernière étape**, sa place restant tenue : sans cela « Précédent » glissait vers le bord droit en arrivant à l'export, et sautait de nouveau en revenant |
| 2026-09-05 | ⚠️ **Une case se cale sur les pixels par ses bords, jamais par sa taille.** `QRectF.toRect()` arrondit la position et la taille **séparément** : deux cases voisines se rejoignaient à un pixel près, et sans écart des lignes du fond traversaient la mosaïque, changeant de place à chaque cran de zoom. Mesuré : **73 des 74 hauteurs essayées** en montraient, jusqu'à 280 pixels de fond à l'intérieur du dessin. En arrondissant les **deux bords**, le bord droit d'une case tombe sur le bord gauche de la suivante, qui est le même nombre |
| 2026-09-05 | **Trois défauts du caméléon et de la loupe, tous vus à l'écran.** ⚠️ **Une ligne blanche d'un pixel longeait chaque carte** : les rectangles se calculent en flottants et se dessinent en entiers, et entre l'arrondi de la carte et celui de l'écart il restait par endroits le fond de la feuille. Les écarts se peignent **avant** les cartes : on les élargit d'un pixel de chaque côté, et les cartes recouvrent le débord. ⚠️ **Deux cases voisines dans la grille ne le sont pas toujours sur le papier** : à la coupe, la place qui reste peut faire plusieurs centimètres, et le dégradé s'y étirait en une large bande. L'aperçu vérifie désormais la **feuille**, comme l'export le faisait déjà, et borne l'écart dessiné à l'écart **réglé**. ⚠️ **Un pavé tactile envoie aussi des événements de molette** : le glissement à deux doigts zoomait au lieu de promener l'image. On le reconnaît à son delta **en pixels**, qu'une molette crantée ne donne jamais, et le **pincement** arrive en geste natif, jamais en molette. Le pourcentage devient un **champ** : atteindre 400 % au bouton demandait sept clics |
| 2026-09-05 | ⚠️ **Le mode caméléon : les vides prennent la couleur de ce qui les borde.** Un écart entre deux cartes devient un **dégradé** tiré d'un bord à l'autre, **pixel par pixel** et non d'une moyenne à l'autre : le raccord suit les motifs, un ciel reste bleu en face du ciel. Une **croisée**, le carré où deux écarts se rencontrent, mélange ses **quatre** coins en bilinéaire, un dégradé à une dimension y faisant une couture avec l'écart voisin. La couleur d'une **case vide** compte comme celle d'une carte. Deux interrupteurs séparés : les **écarts**, et le **pourtour de la grille**, une bande large comme l'écart qui va du bord des cartes au fond. ⚠️ **Le bord se lit d'un seul tenant** : une bande par carte laissait un trou au-dessus de chaque écart, un liseré à créneaux, et les quatre coins laissés au fond faisaient autant d'encoches. Le calcul vit dans `chameleon.py`, et **les deux consommateurs s'en servent** : l'export au pixel, l'aperçu en seize pas que Qt étire. La couleur « Entre les cartes » est **grisée** tant que le caméléon tient les écarts, non retirée : on voit ce qu'on retrouvera. Une **pipette par couleur** prélève dans le dessin, `grab()` donnant exactement ce que l'œil voit, dégradés compris ; elle est **dessinée au QPainter**, ni l'émoji goutte ni les caractères de dessin n'existant dans les polices de tous les systèmes |
| 2026-09-05 | **« Présentation » se lit en trois blocs séparés d'un trait** : la largeur de la carte avec ses deux boutons, l'écart entre les cartes, puis le centrage. ⚠️ Tout à la file, on cliquait « Au plus grand » en croyant agir sur l'écart au-dessus duquel il se trouvait ; les intitulés reprennent leurs mots entiers, la place ne manque plus |
| 2026-09-05 | **La section « Présentation » se resserre.** Deux petits champs sur une seule ligne, « Largeur (mm) » et « Écart (mm) » : les grands compteurs de l'étape 2 prenaient cent cinquante pixels de haut chacun, alors qu'ici on ne pose pas la mise en page, on la retouche. Trois boutons courts, **« Taille réelle »** et **« Au plus grand »** côte à côte, **« Centrer »** seul et centré dessous. « Taille réelle » est neuf : il pose les 63 mm d'une vraie carte, la mosaïque en taille de collection, ce qui n'était atteignable que par le champ |
| 2026-09-05 | **L'étape d'export se réorganise autour de l'image.** Une phrase en gras ouvre l'écran, « Derniers ajustements avant l'export ». Les trois onglets deviennent des **sections rétractables à gauche**, **une seule ouverte à la fois** : deux dépliées font défiler la colonne et l'œil ne sait plus où regarder. Elles gardent une **largeur fixe**, les champs passent en colonne et les paragraphes d'explication disparaissent, un champ un peu large repoussant l'image. À droite, les quatre cases, le résumé et le bouton d'export ; **au milieu, l'image seule**, qui prend tout le reste. Elle se zoome à la **molette autour du curseur** et se promène au **glissement**, avec une loupe posée par-dessus, en bas à droite : une barre en dessous lui aurait repris la hauteur qu'on vient de lui donner. ⚠️ **Le zoom quitte la section « Résolution »** : il porte sur l'image, pas sur le fichier à écrire, et se trouvait à trois clics de ce qu'il agrandit. ⚠️ **Le glissement ne déplace plus un bout de grille ici** : `PagePreview` ignore le clic quand elle n'a rien à poser, sans quoi son cadre ne recevait jamais le geste. ⚠️ **Et les gestes se prennent sur l'aperçu, pas sur le cadre** : c'est lui que la souris survole, si bien que ni le clic ni la molette n'arrivaient au cadre qui les écoutait, et rien ne bougeait. Le cadre l'écoute donc par un **filtre d'événements**, l'écart de glissement se lit en coordonnées **écran**, celles du widget bougeant avec lui à mesure qu'il défile, et l'aperçu porte la **croix directionnelle** pour dire qu'il se déplace |\n| 2026-09-05 | ⚠️ **Un agencement s'enregistre pour de bon, et se partage.** L'idée d'une **graine** est abandonnée : rejouer un calcul de plusieurs minutes exigeait les mêmes cartes, les mêmes réglages, aucun arrêt sur le temps et la même machine, et le dernier bit d'un flottant suffisait à faire diverger la suite. On garde donc le **résultat**. Un fichier JSON par agencement : la **liste des cartes employées**, désignées par chemin comme dans les préréglages, une grille qui les référence par rang, `-1` pour une case vide, et l'**habillage** réglé pour l'imprimer. 441 cartes tiennent en 25 Ko. Les quatre cases restent le choix du moment ; **tout ce qu'on enregistre part aussi dans une bibliothèque**, à côté des préréglages, sous un nom posé sans rien demander, « 21 × 21, 2026-09-05 18:40, score 57658 », renommable. Un **menu** les montre toutes : aperçu de la mosaïque avec une **croix rouge** sur les cartes absentes, liste des cartes employées les manquantes en tête, et les gestes, mettre dans une case, exporter en JSON, importer, renommer, supprimer avec confirmation. ⚠️ **Un agencement auquel il manque une carte ne s'ouvre pas** : amputé, ce n'est plus celui qu'on a partagé ; il se montre et se dit, mais ne passe pas à l'export. ⚠️ `CardSet.subset` **trie** les indices : la grille se retraduit au retour, sans quoi la mosaïque montrerait les bonnes cartes aux mauvaises places |
| 2026-09-05 | ⚠️ **Repolir un widget ne le repeint pas**, et `theme.mark` ne le demandait nulle part. Qt recalculait le style et laissait à l'écran les pixels d'avant : le cadre vert d'une case choisie restait sur la précédente, et seul un événement étranger, un survol, un redimensionnement, finissait par le déplacer. Trois versions de la règle d'allumage ont été écrites avant de trouver que le défaut n'était pas là. ⚠️ **Rien ne le montrait dans les tests** : `grab()` redessine tout, toujours, et l'écran capturé était donc juste alors que l'écran réel ne l'était pas. La non-régression compte les tours de dessin d'un widget **posé à l'écran**. ⚠️ Le repeint passe par `QWidget.update(cible)` et non `cible.update()` : une vue de liste a son propre `update(index)`, qui masque celui du widget et réclame un argument |
| 2026-09-05 | **Quatre agencements gardés au lieu de cinq**, et le cadre vert ne dit plus que la **case choisie**. Quatre cases tiennent en colonne sans la faire défiler, et la cinquième n'ajoutait rien à comparer. ⚠️ **Le cadre s'allumait tout seul**, d'abord à la sauvegarde, puis sur le cliché affiché : dans les deux cas personne ne savait plus ce qu'il désignait, et il restait allumé, le cliché gardé étant celui qu'on venait de regarder. Il ne s'allume désormais **que sur un clic de case**, à l'exécution comme à l'export, et ne s'éteint que si l'on en choisit une autre ou si la sienne est vidée. Garder n'est pas choisir |
| 2026-09-05 | **La colonne des agencements gardés se règle sur trois points.** ⚠️ **Le cadre vert dit une case ouverte, pas une case gardée.** Il s'allume quand on va **sur** un agencement, en cliquant sa case ou en ouvrant sa fenêtre, et s'éteint dès que l'écran montre autre chose. Allumé par la sauvegarde, il ne s'éteignait plus : le cliché gardé est celui qu'on venait de regarder. Le cliché se reconnaît par son **numéro d'itération et sa grille**, deux calculs pouvant passer par la même itération sans y ranger les mêmes cartes. À l'export, même règle : la case cerclée est celle dont l'agencement est affiché. ⚠️ **Un agencement que la timeline n'a plus s'ouvre dans une fenêtre à part**, zoomable, sous un avertissement ambre « L'agencement n'est plus dans la timeline » : le curseur ne bouge pas, le montrer au plus proche donnerait à voir autre chose que ce que la case promet. ⚠️ **La croix demande confirmation** : elle est à quelques pixels de la vignette, et ce qu'elle efface ne se retrouve pas, aucun calcul ne redonnant deux fois le même agencement |
| 2026-09-05 | **Gromago-ex au-dessus de Mordudor**, `b2a-114` et `b2a-096`, quatrième lien fourni d'office et second lien vertical : la forme évoluée domine, comme la lignée d'Arcko |
| 2026-09-05 | **L'export quitte pour de bon la vue d'exécution**, et trois défauts de la mise de côté sont corrigés. Le bouton « Exporter ce cliché » disparaît : l'étape 4 écrit, l'exécution calcule. Toute la machinerie suit, dialogue, barre de progression, annulation, et surtout l'**arrêt du fil** avec ses garde-fous : une référence n'est lâchée que si son fil est réellement terminé, sans quoi la fenêtre se ferme dessus et Qt abandonne le processus. `RunStep.shutdown` n'a donc plus qu'un fil à arrêter. ⚠️ **« Enregistrer » se réveille au premier cliché**, et non au démarrage du calcul, où la timeline est encore vide : il restait éteint tout le calcul, et seul un « Prolonger » le rallumait. ⚠️ **Relancer ne vide pas la colonne** : ce qu'on a mis de côté porte sa grille et son jeu de cartes, un calcul neuf ne peut pas le refaire. ⚠️ **Les cases défilent** : cinq d'affilée réclament sept cents pixels de haut, et posées telles quelles elles imposaient cette hauteur à tout l'écran, jusqu'à ce que l'image du calcul ne tienne plus. La vignette prend enfin toute la hauteur de sa case, la croix étant **posée par-dessus** au lieu de lui manger un rang |
| 2026-09-05 | ⚠️ **Une quatrième étape : « Export ».** L'application passe de trois écrans à **quatre**, Cartes, Paramètres, Exécution, **Export**. On y arrive avec au moins un agencement gardé, et **la première case s'ouvre d'office** : un écran d'habillage sans agencement n'aurait rien à montrer, et la colonne n'est pas un menu qu'on doit penser à ouvrir. Au centre, **les feuilles portent l'agencement pour de vrai**, vignettes comprises : le fil de fer ne dit rien d'une couleur de fond. À droite, les **mêmes cinq cases** qu'à l'exécution, sans leur croix, retirer sous ses propres pieds l'agencement affiché n'apportant rien. Trois onglets, du même style que l'étape 2 mais **sans validation**, un état neutre plutôt qu'une coche verte : **« Présentation »**, largeur d'une carte, écart, feuilles et centrage ; **« Couleurs des vides »**, autour de la grille, entre les cartes, cases vides ; **« Résolution »**, la finesse, le nombre de pixels par feuille et un zoom dans une zone défilante. ⚠️ **La forme de la grille ne s'y règle plus** : la changer déferait l'agencement trouvé. ⚠️ **La couleur des écarts n'est pas celle du fond** : l'export peint l'étendue de la mosaïque avant d'y coller les cartes, si bien qu'elle ne se voit que dans les écarts ; `PosterSettings.gap_colour` vaut `None` par défaut, ce qui s'en remet au fond, la ligne de commande ne connaissant qu'une couleur |
| 2026-09-05 | ⚠️ **L'utilisateur garde jusqu'à cinq agencements avant l'export.** Un bouton « Enregistrer » range le cliché affiché dans la première case libre, et une **colonne de cinq cases** longe l'image : une vignette de la mosaïque, un clic pour y retourner, une croix pour la vider. **« Suivant » ne s'allume qu'à partir d'une case gardée** : l'export part des agencements retenus, pas du cliché affiché. ⚠️ **Le jeu de cartes fait partie de l'agencement** : la grille indexe le sous-ensemble retenu au calcul, et sans lui les mêmes nombres désigneraient d'autres cartes dès que la sélection change ; c'est une référence, l'objet est partagé avec la timeline. ⚠️ **Une case garde son rang** : vider la deuxième ne fait pas remonter la troisième, une colonne qui se réordonne sous la souris ferait cliquer sur autre chose que ce qu'on visait. La sixième sauvegarde est **refusée**, rien n'est écrasé sans qu'on le demande. ⚠️ **Le rang d'un cliché ne survit pas à l'élagage** de la timeline : une case retrouve le sien par son **numéro d'itération**, et à défaut montre l'agencement gardé sans bouger le curseur |
| 2026-09-05 | **L'exécution s'ouvre sur un bouton « Lancer le calcul » au milieu de l'écran**, tant que rien n'a tourné. Une phrase d'attente seule laissait chercher où l'on lance : le « Lancer » de la barre du bas se perd dans une rangée de six boutons, tous éteints sauf lui. L'accueil cède la place à l'image dès le premier cliché, et le geste est le même que celui de la barre. Une **pile** plutôt qu'un texte posé dans le label de l'image : un bouton ne se met pas dans un `QLabel`, et l'accueil n'a que faire d'une zone défilante. ⚠️ Au passage, **« Pause » et « Arrêter » naissaient actifs**, cliquables sans effet tant qu'aucun calcul ne tourne |
| 2026-09-05 | **L'onglet des paramètres se lit en trois parties encadrées**, et la métrique se montre à deux cartes. Trois blocs de texte à la suite se lisaient comme un seul : chaque partie a désormais son cadre, **« La métrique »**, **« Fréquence de sauvegarde »**, qui ne porte que la ligne des clichés, et **« Paramètres de l'algorithme »**. La description de la métrique dit ce qu'elle **est**, un score tiré des bandes voisines de deux cartes, et ce que l'algorithme en fait, déplacer les cartes pour le meilleur score d'ensemble ; « Épaisseur » devient « Épaisseur des bandes ». ⚠️ **L'aperçu montre deux cartes côte à côte, et une flèche à deux pointes entre les bandes qui se touchent** : une carte seule surlignait quatre bandes sans dire à quoi elles servaient, alors que le score n'existe qu'**entre** deux cartes |
| 2026-09-05 | **La métrique rejoint les paramètres, et l'onglet « Recherche » disparaît.** Il ne portait que l'épaisseur des bandes et son aperçu ; séparée des réglages qui s'en servent, la mesure obligeait à l'aller-retour pour comprendre l'un par l'autre. L'onglet unique se lit maintenant en deux temps : **« La métrique »**, son explication sur toute la largeur puis l'épaisseur à gauche et l'aperçu à droite, la cause et l'effet sur la même ligne ; puis **« Paramètres de l'algorithme »**, le paragraphe de vocabulaire et les phrases. ⚠️ **L'algorithme n'a plus de famille dans le menu** : un intitulé pliant qui promet une liste n'aurait qu'un enfant à plier, et son trait ne relierait rien. **Chaque ligne d'arrêt porte l'aura de son état**, comme « Suivant » : verte quand elle compte, rouge et plus discrète quand elle est éteinte. ⚠️ Il lui faut un **cadre opaque** à border, une aura posée sur un fond transparent ne halo que les lettres une par une ; et les constantes de l'aura passent dans `theme` avec `set_glow`, deux réglages du même signal auraient fini par diverger |
| 2026-09-05 | ⚠️ **« Paramètres avancés » devient un texte, et les réglages vivent dedans.** Sept fiches « titre, paragraphe, champ » se lisaient comme sept décisions à prendre avant de pouvoir lancer quoi que ce soit. L'onglet s'ouvre maintenant sur un **avertissement ambre** : les valeurs par défaut suffisent presque toujours, on peut passer directement à la suite. Vient un paragraphe qui explique l'algorithme avec ses mots en gras, **itération**, **échange**, **métrique**, **retenu**, **agencement**, **arrêt**, et ces mots-là sont **ceux que les réglages réemploient** : le vocabulaire s'apprend une fois. Chaque réglage est ensuite un mot dans une phrase : « L'algorithme utilisé est *le recuit simulé* », « il a fait *1 000 000* itérations », « un agencement sera enregistré tous les *10* échanges retenus ». Les arrêts se cochent ligne à ligne, et celui des **itérations est coché et non débrayable** : les autres ne font que couper plus tôt, un calcul sans borne n'existe pas. ⚠️ **Les mots d'une ligne à cocher appartiennent à la case** : une case nue n'offre que douze pixels à viser, et cliquer la phrase ne ferait rien ; l'exception est la case désactivée des itérations, qui griserait la phrase décrivant le réglage actif. La phrase de la tolérance se grise avec son champ en descente stricte, elle promettrait sinon un comportement que le calcul n'aura pas. ⚠️ Une phrase se traduit **en une seule chaîne** portant `%1`, coupée à l'affichage autour de son champ, sans quoi l'ordre des mots serait figé sur le français ; et seule la **fin** de phrase se replie, le début restant collé à son champ, sinon une ligne un peu longue faisait apparaître une barre de défilement horizontale |
| 2026-09-05 | ⚠️ **Aucun tiret cadratin nulle part.** Ni « — » ni « – », dans aucun texte du projet : interface et traductions, commentaires et docstrings, messages de commit, `SPEC.md`, `TODO.md`, `README.md`, et les réponses de l'assistant. La ponctuation ordinaire dit la même chose : deux-points pour annoncer, virgule pour l'incise, parenthèses pour l'aparté, point pour couper. Le trait d'union des mots composés et les puces de liste ne sont pas concernés. La règle est écrite dans `CLAUDE.md`, section « Écriture des textes ». Le nettoyage a touché **661 occurrences** dans 65 fichiers ; toute nouvelle occurrence est une régression. ⚠️ Une passe de remplacement à l'échelle du fichier avait mangé des tranches numpy (`a[:, None]`) et des formats (`{n:,}`) sur des lignes sans tiret : la correction ne touche **que** les lignes qui en portent un. Deux textes d'interface changent de forme plutôt que de ponctuation : le compteur de clichés affichait « — » faute de timeline, il dit maintenant « aucun cliché », et les cellules de tableau qui portaient un tiret nomment ce qu'elles veulent dire (« aucun », « impossible ») |
| 2026-09-05 | ⚠️ **L'algorithme rejoint les paramètres, et l'étape 2 se renomme.** Il avait son écran à lui, entre la mise en page et l'exécution ; il n'avait pourtant rien de plus à décider, ce sont des paramètres comme le papier et la grille, et les séparer obligeait à traverser une étape entière pour revenir changer une durée. L'application passe de quatre étapes à **trois** : Cartes, **Paramètres**, Exécution. La colonne d'onglets porte désormais **deux familles** : « Grille » et « Algorithme », chacune avec son trait, un trait unique aurait traversé le second intitulé en prétendant que tout descend du premier. Deux onglets : **« Recherche »**, l'épaisseur des bandes et son aperçu, seul réglage de l'algorithme qui se **voie** ; **« Paramètres avancés »**, les sept autres, chacun **précédé de son explication**, ce que c'est, à quoi ça sert, ce que ça change, chiffres mesurés à l'appui. ⚠️ Le texte fait partie du réglage : « tolérance d'acceptation : 0,30 » ne dit rien à personne, pas même à qui a écrit le programme six mois après |
| 2026-09-05 | **Les cases vides sont déjà transparentes pour l'algorithme**, vérifié, non modifié. `grid_score` écarte toute paire dont l'une des deux cases est vide, `local_score` fait de même, l'optimiseur ne prend jamais une case vide comme source et ne recouvre jamais un trou figé. Un trou est donc un **bord** : il ne coûte rien, ne rapporte rien, et coupe simplement le voisinage. Sa couleur n'est que d'affichage, et quitte les réglages pour l'étape d'export à venir |
| 2026-09-05 | **Les « grilles qui tiendraient » disent où est la sortie quand aucune ne convient** : un avertissement ambre en gras sous la liste, « ajoutez des feuilles pour qu'aucune carte ne reste en trop ». Une proposition qui laisse des cartes dehors n'en est pas vraiment une, c'est la meilleure à cette taille de carte, mais l'appliquer amputerait le poster, et rien ne le disait. ⚠️ **Il ne paraît que si elles laissent toutes des cartes dehors**, et non dès que l'une le fait : les propositions sont classées par nombre de cartes placées, si bien qu'une seule qui les loge toutes passe en tête et que les plus petites qui suivent en laissent forcément, avertir sur l'une d'elles reviendrait à avertir presque toujours, y compris quand le premier choix tombe pile poil |
| 2026-09-05 | **L'onglet des tailles dit aussi quand des cartes sortent des pages**, en une seconde phrase rouge et bloquante, qui s'ajoute à celle du compte des cases. La place sur le papier appartient à l'onglet des tailles de carte, qui la règle ; mais c'est en **agrandissant la grille** qu'on la fait déborder, et l'onglet où l'on agrandit ne peut pas se taire sur ce qu'il vient de casser. ⚠️ Chaque phrase garde **sa** couleur : une teinte unique pour tout le bloc peignait en rouge « la grille a exactement autant de cases que de cartes », qui est pourtant une bonne nouvelle |
| 2026-09-05 | ⚠️ **« Suivant » regarde tout le parcours, pas seulement la partie affichée.** Une partie déjà validée qui casse : la grille agrandie déborde des pages, une carte de plus fait sauter le compte, bloque le bouton **où qu'on soit**, et sa croix rouge dans la colonne dit laquelle. On revenait sinon en arrière, on cassait quelque chose, on repartait par un onglet plus loin, et l'on quittait l'étape avec une mise en page impossible. Les parties **pas encore validées** ne comptent pas : au premier passage la grille par défaut ne tient presque jamais, et les exiger fermerait le chemin qui mène justement à l'onglet où la corriger |
| 2026-09-05 | **« Suivant » porte une aura de son état** : verte quand il est prêt, rouge et **plus discrète** quand il manque quelque chose. Une aura et non un fond coloré : le bouton garde l'apparence d'un bouton, et c'est autour de lui que se lit l'état. Qt ne connaissant pas `box-shadow`, c'est un `QGraphicsDropShadowEffect` sans décalage, et la barre du bas réserve la place qu'il déborde. ⚠️ **Rien du tout à la dernière étape** : le bouton y est éteint parce qu'il n'y a plus d'écran après, non parce qu'il manque quelque chose, une aura rouge y accuserait un travail qui est fini. Le rouge est moins vif que le vert : il accompagne le premier passage sur chaque écran, et aussi franc que l'invitation à continuer, il donnerait au parcours l'air d'une suite d'erreurs. ⚠️ **Ce qui est peint une fois ne suit pas le mode** : tout ce qui se lit au moment du dessin bascule tout seul, mais une couleur figée dans un effet ou dans une image reste celle du mode d'alors. La fenêtre et l'étape 2 écoutent `PaletteChange` pour refaire l'aura et les pastilles |
| 2026-09-05 | **La couleur d'état d'un onglet passe du texte au cadre.** Un libellé coloré perdait le contraste que le mode lui donne ; le fond et le contour, eux, ne font que teinter le gris de départ, le contour beaucoup (70 %), le fond peu (16 %), et le texte reste noir ou blanc selon le mode. Les onglets **verrouillés** gardent leur gris : leur couleur dirait un état sur lequel on ne peut rien. Une feuille de style ne sachant pas viser une ligne, c'est `TabDelegate` qui peint le cadre, et `::item` ne garde que ses mesures. La sélection n'est plus le bleu de Qt mais le même cadre en plus fort, sinon l'onglet ouvert serait le seul à ne pas montrer son état. Contrastes **calculés** pour les trois états, les deux modes et les deux intensités : à 34 % de teinte, la pastille rouge tombait à 2,6:1 sur son propre fond rouge, sous le seuil de 3:1 des éléments non textuels, la part de l'onglet ouvert est ramenée à 24 %, et celle des autres à 12 %, le contour plein et plus épais disant surtout la sélection. ⚠️ **Découvert en le faisant** : `is_dark()` lisait *sombre* en mode clair sur tout widget dont une feuille de style pose `background: transparent`, Qt lui donne alors un `Window` noir d'alpha nul. Le repli sur la palette de l'application corrige les onglets **et** le trait qui relie le second rang, qui prenait déjà la mauvaise teinte |
| 2026-09-02 | **Les extensions se rangent par série.** « A1 », « A1a » et la promo A sont la **série A** : c'est ainsi qu'on cherche une extension, par série d'abord. La liste plate obligeait à lire chaque nom pour savoir où l'on en était, et il y en aura d'autres lettres. La série se lit dans le nom du dossier, le miroir les nomme `a1-puissance-genetique`, `promo-a-promo-a`, rien d'autre ne la donne, le catalogue ne connaissant que des extensions. Une série se sélectionne comme une extension et **vaut toutes les siennes** : « Inclure l'extension » inclut alors toute la série. Un dossier choisi à la main ne suit aucune convention : il ne rejoint aucun groupe et se pose tel quel |
| 2026-09-02 | **L'onglet des pages ne dit plus la part de papier couverte.** Le message donnait le pourcentage et prévenait que le reste sortirait blanc. Il disait vrai, mais il s'affichait dès qu'on ajoutait une feuille, au moment précis où l'on demande de la place, et il fallait le lire à chaque fois pour n'en rien faire. Ce que la mosaïque couvre se voit sur le dessin, et se règle aux onglets « Grille » |
| 2026-09-02 | ⚠️ **La coupe se dessine en deux tons, et chaque feuille porte son numéro.** Un trait unique la rendait invisible : tiré dans la couleur du texte, il passait en **blanc sur une mosaïque bleu très clair**, et l'on ne voyait plus où le poster se coupe, la seule chose que ce dessin ait à dire de plus qu'une image du résultat. Un trait clair plein de 4 px surmonté d'un pointillé sombre de 2 px est la convention des repères d'imprimerie : l'un des deux ressort quel que soit le fond, dans les deux modes. Le numéro de chaque feuille est celui de son fichier ; il se glisse **dans la marge** du morceau quand il y en a une, et ne prend un fond que sur une feuille pleine à ras bord. Écarte la gouttière, écarter les feuilles à l'écran est plus lisible encore, mais l'aperçu cesserait d'être à l'échelle exacte de l'export. **Découvert en le faisant** : le fil de fer de la vue d'exécution **ignorait les déplacements** de morceaux, et peignait un rectangle de papier blanc en travers de la coupe, la mosaïque n'étant plus d'un seul tenant |
| 2026-09-02 | **Un bouton « Centrer »**, geste inverse du glissement : chaque morceau se pose au milieu de **sa** feuille, dans les deux sens. ⚠️ Les feuilles sans carte sont laissées de côté, la dernière n'en porte souvent aucune, et lui inventer une position la ferait compter parmi les feuilles déplacées. Le bouton s'éteint quand tout est déjà centré, ce qui arrive sans qu'on y touche : sur une feuille remplie à ras bord, le placement par défaut **est** le centre |
| 2026-09-02 | **Le troisième onglet déplace la grille, feuille par feuille.** On garde le clic sur la mosaïque et on tire. ⚠️ **Un morceau ne quitte pas sa feuille** : le poster se coupe entre deux cartes, jamais au milieu d'une, et laisser glisser la mosaïque d'une feuille à l'autre remettrait cette règle en jeu à chaque geste. Chaque feuille porte donc son morceau et le déplace **pour son compte**, borné à ce qui lui reste de libre. Cliquer dans le blanc d'une feuille n'attrape rien : on ne déplace que ce qu'on voit bouger. Toute modification venue des onglets d'avant, taille de grille, format, feuilles, taille de carte, **défait les déplacements** : le morceau de chaque feuille n'a plus la même taille, et la place qu'on lui avait choisie ne veut plus rien dire. Un bouton « Remettre en place » fait la même chose à la demande. ⚠️ **Conséquences sur le calcul** : `column_x` et `row_y` fusionnent en `PosterPlan.card_origin`, la position d'une case dépendant désormais de sa ligne **et** de sa colonne ; la position d'un morceau est bornée des **deux** côtés, à l'écran qui laisse glisser et à l'export qui écrit, un préréglage écrit à la main suffirait sinon à pousser une coupe en pleine carte ; et la marge par défaut se calcule sur ce que la **feuille** loge, non sur ce que la grille compte, sans quoi une grille qui déborde donnait une marge négative à l'export et positive à l'écran, deux dessins du même poster |
| 2026-09-02 | **Les feuilles se posent aussi en lignes**, cinq colonnes et trois lignes au plus, soit quinze feuilles. Un « + » au-dessus du dessin ajoute une ligne comme celui de droite ajoute une colonne, et un « − » se pose à gauche de chaque ligne comme il s'en pose un sous chaque colonne : le même geste sur deux axes. ⚠️ **La règle de la coupe vaut dans les deux sens** : `GridGeometry` porte désormais `rows_per_panel` à côté de `per_panel`, et `PosterPlan.row_y` fait repartir chaque ligne de feuilles de son **bord haut**, comme `column_x` le fait des colonnes. La marge verticale ne se centre donc que sur une **seule** ligne de feuilles ; au-delà elle passe à zéro, faute de quoi elle pousserait la dernière ligne de chaque feuille au-delà de son bord bas, une coupe en pleine carte, la seule chose qu'on ne s'autorise jamais. Les fichiers portent leur place dans le mur (`poster_l2c3sur2x3.png`), les repères de coupe et le chevauchement débordent des quatre côtés, et une ligne de plus **agrandit les cartes** quand la hauteur contraignait. ⚠️ **Corollaire** : `grid_geometry` rend **zéro** carte par feuille quand rien ne tient, au lieu d'un plancher à un, une carte de deux mètres en logeait « une », et `grid_fits` répondait que la grille d'une colonne tenait |
| 2026-08-31 | **Le parcours de l'étape 2 se fait dans l'ordre.** Un onglet ne s'ouvre que lorsque **toutes** les parties d'avant ont été validées au moins une fois par « Suivant » ; les autres sont grisés, inertes au clic, et disent en infobulle ce qui les ouvrira. ⚠️ Sauter d'un clic à l'onglet des écarts sans avoir vu les pages ni la grille, c'est régler la taille des cartes pour un papier qu'on n'a pas choisi : les onglets qui suivent dépendent de ce que ceux d'avant décident, et rien à l'écran ne le disait. Une seule visite validée suffit, **définitivement** : on revient en arrière autant qu'on veut, et changer d'avis ne referme rien, c'est la pastille, non le verrou, qui dit si une partie tient toujours debout. La pastille d'un onglet verrouillé s'estompe : Qt éteint le libellé d'une ligne désactivée, mais pas une icône qu'on dessine soi-même, et elle réclamait l'attention pour une partie sur laquelle on ne peut rien |
| 2026-08-31 | **Electron écarté ; l'application reste en Qt.** Le problème qu'il résout n'existe pas ici, Qt tourne déjà sur les trois systèmes, et celui qui reste, fabriquer le paquet sur chacun, est identique avec lui : une machine par système, notarisation, SmartScreen. Le cœur est du numpy vectorisé : il faudrait ou le réécrire en JavaScript, ou livrer **deux** environnements d'exécution, Chromium et Python. L'export sortirait du navigateur de toute façon, le canvas de Chromium refusant au-delà de ~16 000 px de côté quand un A0 à 600 dpi en fait 19 866. Coût mesuré : 7 900 lignes d'interface et 6 100 lignes de tests à refaire, cinq widgets dessinés au `QPainter` compris, pour revenir au même endroit et sans gain de poids, le `.app` fait déjà 118 Mo. Tauri tombe sous les mêmes objections : il change le navigateur embarqué, pas la place de Python. **Rouvrir la question demanderait que le but change**, composer la mosaïque dans un navigateur, sans installation, partageable par un lien |
