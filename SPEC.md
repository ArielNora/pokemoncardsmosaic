...
888 tests. Interface auditée en entier, en deux lots, `ui/` d'abord, puis la vue
...
| 2026-09-05 | ⚠️ **Le calcul a une graine, et devient reproductible.** `optimize_grid` retombait sur le hasard **global** : deux calculs identiques donnaient deux posters, et aucun ne se rejouait. Un seul générateur, semé une fois, sert désormais au tirage de la grille de départ **et** à toute l'optimisation. Laissée vide, la graine est tirée à chaque calcul et **retenue** : l'exécution l'affiche et la copie, pour la relire ou la partager. Fixée, elle rejoue **exactement** le même calcul, clichés compris, ce qui est mesuré par un test qui compare deux timelines entières. Elle part dans les préréglages, pas `last_seed`, qui décrit ce qui a eu lieu et non ce qu'on a réglé. ⚠️ **Ce que la graine ne garantit pas** : un arrêt sur le **temps**, une pause ou un « Arrêter » coupent à un instant qui ne se reproduit pas ; un partage suppose **les mêmes cartes** et les mêmes réglages, donc le même catalogue, la renumérotation des cartes dépendant de la sélection ; et d'une machine à l'autre, le dernier bit d'un calcul flottant peut faire basculer une acceptation. La reproduction est donc **exacte chez soi**, et très probable ailleurs |
